    ...
    , mDeleteAction()
{
...

    {
        ...
        mObjectAction = new QAction(tr("Create a layer object"), this);
        mObjectAction->connect(mObjectAction, &QAction::triggered, this, &ObjectTreeWidget::onObjectActionTriggered);

        mFolderAction = new QAction(tr("Create a folder object"), this);
        ...
        if (dialog->hasValidNode())
        {
			...
			for(int i = 0; i < node_count; i++){
				...
				cmnd::ScopedMacro macro(mProject->commandStack(),
                                        CmndName::tr("Create a layer object"));  // Unsure if it's "layer object" or "layered object"
				// notifier
				{
